    ...
    <div className="w-full rounded-2xl p-3 border border-slate-800 bg-slate-900">
      <div className="flex items-center justify-between gap-2 mb-2">
        <div className="flex items-center gap-1.5">
          <Clock className="w-4 h-4 text-hh-emerald" />
          <span className="text-xs font-bold text-white">Live Stage Latency HUD</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-xs font-mono font-bold text-gray-200">
            Total: <strong className={isSub200 ? "text-hh-emerald font-extrabold" : "text-hh-gold font-extrabold"}>{total_ms.toFixed(1)} ms</strong>
          </span>
          <span className={`px-2 py-0.5 text-[9px] font-bold rounded-full uppercase ${isSub200 ? 'badge-grounded' : 'badge-gold'}`}>
      ...
      <div className="w-full h-2.5 bg-slate-950 rounded-full overflow-hidden flex mb-2.5 p-0.5 gap-0.5 border border-slate-800">
        <div style={{ width: `${sttPct}%` }} className="h-full bg-hh-coral rounded-sm" title={`STT (Sarvam): ${stt_ms.toFixed(1)}ms`}></div>
        <div style={{ width: `${retPct}%` }} className="h-full bg-hh-emerald rounded-sm" title={`Retrieval (Qdrant+BM25): ${retrieval_ms.toFixed(1)}ms`}></div>
        <div style={{ width: `${genPct}%` }} className="h-full bg-hh-gold rounded-sm" title={`Generation (LLM): ${generation_ms.toFixed(1)}ms`}></div>
